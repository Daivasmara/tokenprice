...
const { argv } = yargs
  ...
  `)
  .example(`$0 ${STORE_API_KEY} dummykey123`, '\'dummykey123\' will be stored in your local machine .env.')
  .example(`$0 ${FETCH} -t btc eth dgx -c idr`, 'show prices for BTC, ETH and DGX in IDR currency.')
  .epilogue(`README: ${README_URL}`)
  .command(`${STORE_API_KEY} [key]`, 'to store API key in your local machine .env.', (y) => {
    y
      .positional('key', {
        describe: 'this API key will be stored in your local machine .env (<global_node_modules>/TOKENPRICE/.env).'
